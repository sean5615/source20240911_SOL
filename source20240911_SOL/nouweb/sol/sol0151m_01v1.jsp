<%/* 
----------------------------------------------------------------------------------
File Name        : sol0151m_01v1
Author            : barry
Description        : SOL0151M_收件資料登錄審查 - 顯示頁面
Modification Log    :
...
session.setAttribute("SOLT007_02_SELECT", "NOU#select CODE AS SELECT_VALUE,CODE_NAME AS SELECT_TEXT from SYST001 where KIND='EDUBKGRD_GRADE' ORDER BY SELECT_VALUE, SELECT_TEXT");
session.setAttribute("SOLT007_03_SELECT", "NOU#select CODE AS SELECT_VALUE,CODE_NAME AS SELECT_TEXT from SYST001 where KIND='VOCATION' ORDER BY SELECT_VALUE, SELECT_TEXT");
session.setAttribute("SOLT007_041_SELECT", "NOU#select CONCAT(a.FACULTY_CODE, a.TOTAL_CRS_NO) as SELECT_VALUE, a.TOTAL_CRS_NAME as SELECT_TEXT from syst008 a where a.ASYS = '1' and a.FACULTY_CODE != '90' ORDER BY SELECT_VALUE, SELECT_TEXT");
session.setAttribute("SOLT007_042_SELECT", "NOU#select CONCAT(a.FACULTY_CODE, a.TOTAL_CRS_NO) as SELECT_VALUE, a.TOTAL_CRS_NAME as SELECT_TEXT from syst008 a where a.ASYS = '2' and a.FACULTY_CODE != '90' ORDER BY SELECT_VALUE, SELECT_TEXT");
session.setAttribute("SOLT007_05_SELECT", "NOU#select CODE AS SELECT_VALUE,CODE_NAME AS SELECT_TEXT from SYST001 where KIND='TUTOR_CLASS_MK' and CODE = '1'  ORDER BY SELECT_VALUE, SELECT_TEXT");
...
session.setAttribute("SOLT007_12_SELECT", "NOU#select CENTER_CODE AS SELECT_VALUE,CENTER_NAME AS SELECT_TEXT from SYST002 WHERE CENTER_CODE = '" + request.getParameter("CENTER_CODE") + "' ORDER BY SELECT_VALUE, SELECT_TEXT");
...
session.setAttribute("SOL015M_01_BLUR", "NOU#SELECT a.ZIP CRRSADDR_ZIP, (a.ZIP_CITY||a.ZIP_TOWN) CRRSADDR FROM SYST006 a where a.ZIP = '[CRRSADDR_ZIP]' ORDER BY a.ZIP");
session.setAttribute("SOL015M_01_WINDOW", "NOU#SELECT a.ZIP CRRSADDR_ZIP, (a.ZIP_CITY||a.ZIP_TOWN) CRRSADDR FROM SYST006 a order by a.ZIP");
session.setAttribute("SOL015M_02_BLUR", "NOU#SELECT a.ZIP DMSTADDR_ZIP, (a.ZIP_CITY||a.ZIP_TOWN) DMSTADDR FROM SYST006 a where a.ZIP = '[DMSTADDR_ZIP]' ORDER BY a.ZIP");
session.setAttribute("SOL015M_02_WINDOW", "NOU#SELECT a.ZIP DMSTADDR_ZIP, (a.ZIP_CITY||a.ZIP_TOWN) DMSTADDR FROM SYST006 a order by a.ZIP");
	/**取得行政人員所屬中心*/
...
<head>
    ...
    <script src="sol0151m_01c1.jsp"></script>
    <noscript>
...
<!-- 定義查詢的 Form 結束 -->
        ...
        <form id="EDIT" name="EDIT" method="post" onsubmit="doSave1();" style="margin:5,0,0,0;">
			...
			<input type=hidden name="REG_FEE">
            <input type=hidden name="DRAFT_NO">
            <input type=hidden name="TOTAL_RESULT">
			<input type=hidden name="AUDIT_UNQUAL_REASON1">
                    ...
                    <td width="100%" valign="top" bgcolor="#FFFFFF">
                        ...
                        <table id="table2" width="100%" border="0" align="center" cellpadding="2" cellspacing="0" summary="排版用表格">
                            <tr>
                                ...
                                <td class='tdGrayLight'><input type=text name='AYEAR'></td>
                            ...
                            <tr>
                                <td class='tdgl1' align='right'>學制
                                <select id='ASYS' name='ASYS' disabled>
                                    <option value=''>請選擇</option>
                                    <script>Form.getSelectFromPhrase("SOLT007_13_SELECT", null, null);</script>
                                </select></td>
                                <td align="left" class='tdGrayLight' align='right'>身分證字號<input type=text name='IDNO' id= 'IDNO'>
                                	<br>護照居留證到期日：<input type=text name='RESIDENCE_DATE' size = "8" maxlength = "8" ></td>
                                <td class='tdgl1' align='right'>出生日期</td>
                                <td class='tdGrayLight'><input type=text name='BIRTHDATE' id='BIRTHDATE'></td>
                            </tr>
                            <%
                            String major = (String) request.getParameter("ASYS");
                            %>
                            <tr>
                                <td align='right' class='tdgl2'>報名學習<%= "1".equals(major) == true ? "指導中心" : "輔導處" %><font color=red>＊</font>：</td>
                                <td class='tdGrayLight'>
                                    <select name='CENTER_CODE' disabled>
                                        <option value=''>請選擇</option>
                                        <%if("1".equals(major)){%>
											<script>Form.getSelectFromPhrase("SOLT015_12_SELECT", null, null);</script>
										<%}else if("2".equals(major)){%>
											<script>Form.getSelectFromPhrase("SOLT015_13_SELECT", null, null);</script>
										<%}%>
                                </select></td>
                            ...
                            <tr>
                                ...
                                <td class='tdGrayLight' colspan="1"><input type=text name='ENG_NAME' >(同護照英文名稱)</td>
                                <td align='right' class='tdgl2'>國籍<font color=red >＊</font>：</td>
		                        <td class='tdGrayLight'>
                            ...
                            <tr>
                                <td align='right' class='tdgl1' rowspan="3"><font size="3">報名資格<br>(選修生免填)</font></td>
                                <td class='tdGrayLight' ><font size="3">學歷：
                                <input id="edubkgrd1" type="text" name="edubkgrd1" size="40"></td>
                            ...
                            <tr>
                                <td class='tdGrayLight' colspan="2" align="center"><font size="3">(請填證書上之全銜)</font></td>
                                <td class='tdGrayLight'><font size="3">(畢業、結業、肄業等)</font></td>
                            </tr>
                            <tr>
                                <td class='tdGrayLight' colspan="3"><font size="3">填寫範例：<br>
								1.畢業生例：台灣省立大高級中學 畢業 <br>
								2.結業生例：台灣省立大高級工業職業學校附設高級工業職業進修補習學校 結業 <br>
								3.肄業生例：台灣省立大高級中學 肄業修畢二下課程可升級<br>
								4.技術士證例：丙級技術士證五年以上、乙級技術士證二年以上<br>
								5.考試名稱及類科例：公務人員高等考試、二級考試人事行政科
								</font>
								</td>
                            </tr>
                            <tr>
                                <td align='right' class='tdgl2'>戶籍地址<font color=red>＊</font>：</td>
								<td class='tdGrayLight' colspan="3"><input type='text' Column='DMSTADDR_ZIP' name='DMSTADDR_ZIP' onblur='addr_d2(this.value)'>
								<img src='/images/select.gif' name='bt1' alt='開窗選取' style='cursor:hand' onkeypress='addr_d();' onclick='addr_d();'>
								<input type='text' Column='DMSTADDR' name='DMSTADDR'>
                            ...
                            <tr>
								...
								<td class='tdGrayLight' colspan="3"><input type='text' Column='CRRSADDR_ZIP' name='CRRSADDR_ZIP' onblur='addr2(this.value)'>
								<img src='/images/select.gif' name='bt1' alt='開窗選取' style='cursor:hand' onkeypress='addr();' onclick='addr();'>
								<input type='text' Column='CRRSADDR' name='CRRSADDR'></td>
                            </tr>
                            <tr>
                            ...
                            <tr>                                
                                ...
                                <td class='tdGrayLight'><input type=text name='EMRGNCY_RELATION'></td>
                            ...
                            <tr>
                                ...
                                <td align='right' class='tdgl2' >是否參加導師班<font color=red>＊</font>：</td>
                            ...
                            <tr>
                                ...
                                </select><input type="text" name="GETINFO_NAME" >
                                -->
                                </td>
                            </tr>
                            <tr>
                                <td align='right' class='tdgl1'>身心障礙類別：</td>
                                <td class='tdGrayLight'>
                                    <select name='HANDICAP_TYPE' onChange='changeGrade();'>
										<option value="">請選擇</option>
            ...
            <table width="500" height="27" border="0" cellpadding="0" cellspacing="0">
                <tr>
                    <td background="<%=vr%>images/ap_index_title.jpg" alt="排版用圖示">
                        　　				<span class="title">SOL015M_通訊報名學生資料</span>
                    </td>
                </tr>
            </table>
        </td>
